...
| `SDD_REVISE_CHECKS_EXCLUDE` | unset (empty) | operator (optional) | Comma-separated check-run-name globs (`*` matches any run of characters) that never trigger the implicit `/revise`, in either `SDD_REVISE_ON_CHECKS` mode (issue #259). Use it to keep advisory failures — e.g. an E2E or build matrix — from burning the auto-revise retry budget. Read by every `sdd-execute` tier. |
