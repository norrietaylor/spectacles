...
| `SDD_TRIAGE_MIN_TASK` | `60` | operator (optional) | Estimated-diff floor (net changed lines) below which `sdd-triage` folds a task into a cohesive sibling — one whose `files in scope:` overlap, or that form a strict produce/consume chain — instead of emitting it standalone. Cuts the per-task PR/CI/validate/review/merge overhead an over-split plan pays for sub-100-line work. The estimate is the agent's pre-implementation judgment; cohesion is the gate and this floor only breaks ties, so unrelated small tasks are never merged. Set to `0` to disable bundling and keep one task per requirement; raise it to bundle more aggressively (issue #252). |
