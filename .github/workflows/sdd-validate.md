...
When a proof artifact cannot be re-executed because of an infrastructure limit
— this agent runs inside the same firewalled gh-aw container as the other
agents, with no toolchain for the consumer's language and no egress to its
package registry, so a command such as `cargo test` fails on a registry
403/CONNECT-tunnel error before exercising the change — do not treat the
limitation as a proof-artifact failure. Follow the proof-artifacts gate in the
