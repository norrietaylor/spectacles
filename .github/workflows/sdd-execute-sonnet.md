...
variant runs the `claude-sonnet-4.6` model and selects only tasks carrying the
`model:sonnet` label.
...
§1 as widened by ADR 0024; file count and new public API surface are
...
- This variant's frontmatter declares the `claude-sonnet-4.6` engine model and
