...
they install Node on the host, detect the consumer's package manager from the
lockfile present, refresh the lockfile when `package.json` changed, run the
consumer's own prettier and eslint fixers, amend the agent's commit with
...
Nothing is hardcoded. The package manager is detected from the lockfile present
(`pnpm-lock.yaml` → pnpm, `yarn.lock` → yarn, `package-lock.json` /
`npm-shrinkwrap.json` → npm); a project with no lockfile gets no lockfile
