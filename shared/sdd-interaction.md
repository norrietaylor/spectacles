...
| `sdd:in-progress` | the cascade is armed and tasks are being implemented | `sdd-dispatch` on the first `/dispatch` | every task sub-issue closes |
...
materialization). The `kind:*`, `priority:*`, and
`model:{haiku,sonnet,opus}` labels are orthogonal metadata and are not part
...
| `/dispatch` | tracking issue, in `sdd:ready` or `sdd:in-progress` | arm cascade execution of every task in the feature's task tree |
...
0010). `/dispatch` is the second non-PR decision point: it arms the
event-driven cascade that runs the tasks. Execution is fully event-driven:
`/approve` → `/dispatch` → cascade-on-close (ADR 0011).
